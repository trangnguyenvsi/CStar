...
import com.vsii.cstar.testdata.AddScreening;
...
public class testOnly {
	...
	public void start() throws IOException, InterruptedException {
		...
		// ScreeningMaintainanceMethods(driver);
		// objAptSingleScreenMethod = new
		// AppointmentsSingleScreeningPageMethod(driver);
	...
	@Test(priority = 0, description = "login successfully", dataProvider = "Choose setup screening kind", dataProviderClass = AddScreening.class)
	public void TS01(String isCreateScreeningByExistingSite) {
		...
		if (isCreateScreeningByExistingSite.equals("true")) {
	...
	@Test(priority = 8, dataProvider = "VerifyZipCode", dataProviderClass = AddScreening.class)
	...
	@Test(priority = 9, description = "Select existing site to setup screening", dataProvider = "Existing Site", dataProviderClass = AddScreening.class)
	...
	@Test(priority = 11, description = "To check validation address function", dataProvider = "Validation Address", dataProviderClass = AddScreening.class)
	public void TS12(String address1, String expectMessage) throws InterruptedException {
		...
		if (useExistingSite == false) {
			...
			// first
			if (objSiteInfoMethod.getAddressStatus().equals("Bad Address")) {
				new WebDriverWait(driver, 30)
						.until(ExpectedConditions.visibilityOfElementLocated(By.id("scWizAddrSugContainer")));
				objSiteInfoMethod.closeAddressSuggestion();
				new WebDriverWait(driver, 30)
						.until(ExpectedConditions.invisibilityOfElementLocated(By.id("scWizAddrSugContainer")));
				Thread.sleep(2000);
			}
	...
	@Test(priority = 12, description = "Create new site", dataProvider = "CreateNewSite", dataProviderClass = AddScreening.class)
	...
	@Test(priority = 13, description = "Verify dropdown ACO Type when Marketing Plan is ACO and when Marketing Plan is other choice", dataProvider = "ACO Type Verification", dataProviderClass = AddScreening.class)
	...
	@Test(priority = 14, description = "Save and continue to Screening when all mandatory field is fullfilled and valid", dataProvider = "ScreeningInfoData", dataProviderClass = AddScreening.class)
	public void TS15(String repName, String marketingPlan, String acoType, String driveType, String siteOpenTime,
		...
		// Get delivery point
		String queryGetDeliveryPoint = "select adr.Delivery_point from PL_Addresses_Sites adr "
	...
	// @Test(priority=14, description="prepare")
	// public void Temp(){
	// try {
	// objTeamCalendarMethod.selectScreeningByInputDate("2016-06-01 00:00:00");
	...
	public void TS24() {
		...
		System.out.println("Executing: Testcase TS24: Verify Save & Continue function");
		System.out.println("Completed" + "\n");
	}
	...
	public void TS25() throws IOException, SQLException {
		...
		// Get active tab
		String activeTab = driver.findElement(By.xpath("//span[@class='ajax__tab_active']/span/span/span")).getText();

		// Click button Exit in selected tab
		if (objScreeningMaintainanceMethod.objScreeningMaintainance.getTab_SiteInfo().getText().equals(activeTab)) {
		...
		} else if (objScreeningMaintainanceMethod.objScreeningMaintainance.getTab_Notes().getText().equals(activeTab)) {
	...
	public void TS27() {
		...
		// Enter date needer
		objScreeningMaintainanceMethod.objScreeningMaintainance.getTxt_DateNeeded().sendKeys("6/20/2016");
	...
	public void end() throws InterruptedException, IOException {
		// TODO Delete it when scripts is completed
		// String deleteCheckRequest = "DELETE SC_Check_Requests where
		// Screening_ID=294593";
		...
		String connection = "jdbc:sqlserver://" + dbAddress + ";databaseName=" + dbName + ";user=" + user + ";password="
				+ pwd;
		if (useExistingSite == false) {
			String queryDeleteData = "DECLARE @siteId INT " + "DECLARE @screeningId INT "
					+ "set @siteId = (select Site_ID from SC_Sites where Site_Name = '" + objScreening.getSiteName()
					+ "') " + "set @screeningId = (select Screening_ID from SC_Screenings where Site_ID = @siteId) "
					+ "DELETE SC_Screening_History where screening_id = @screeningId "
					...
					+ "DELETE SC_Site_Reviews where site_id = @siteId "
					+ "DELETE SC_Site_Requirements where site_id = @siteId "
					+ "DELETE SC_Sites where site_id = @siteId";
